...
'''
csv_location = "/bigdata/maruvkalab/guyshani/csv_files/"
image_location = "/bigdata/maruvkalab/guyshani/all_images/"
temp_files = "/bigdata/maruvkalab/guyshani/temp_files/"
output_loc = "/bigdata/maruvkalab/guyshani/output_files/"
'''
...
gene_file = "3000_3500_genes_counts"
...
def label_data_train_and_predict(gene, ensembl, k, batch_size, num_workers, num_epochs, device):
	...
	for j in range(k):
		...
		train_data_loader, valid_data_loader, train_data_size, valid_data_size, image_transforms = load_data(f"{gene}_crossval_trainingset_{j}.csv", f"{gene}_crossval_group_{j}.csv", batch_size, num_workers, csv_location, image_location)
...
with open("pvalues_"+gene_file+".txt", 'w') as final_file:
	...
	with open(gene_file+".csv", "r") as gene_list:
